...
public class GameTree {
    ...
    void traverseTree(TreeNode node, Player player) {
//        drawBoard(node.name);
        if (node.leaf) {
            return;
        }

        if (player == Player.ONE) { // find highest value
            int highest = Integer.MIN_VALUE;
            for (int i = 0; i < node.numChildren; i++) {
                TreeNode currNode = node.children[i];

                traverseTree(currNode, other(player));

                int currNumber = currNode.value;
